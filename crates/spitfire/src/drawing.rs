...
/// Backdrop while `SpitfireState::locked` is set — opaque black, so
/// whatever's mapped underneath the lock surface never shows through even
/// for a single frame (e.g. before the client has drawn anything yet).
pub static CLEAR_COLOR_LOCKED: Color32F = Color32F::new(0.0, 0.0, 0.0, 1.0);
