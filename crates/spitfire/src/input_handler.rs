...
impl<BackendData: Backend> SpitfireState<BackendData> {
    fn process_common_key_action(&mut self, action: KeyAction) {
        match action {
            ...
            KeyAction::LuaBind(idx) => match self.config.invoke_bind(idx) {
                Ok(commands) => {
                    for cmd in commands {
                        self.apply_config_command(cmd);
                    }
                }
                Err(err) => error!(%err, "Lua bind callback raised an error"),
            },
    ...
    fn apply_config_command(&mut self, cmd: ConfigCommand) {
        match cmd {
            ...
            ConfigCommand::LayoutSet(mode) => {
                info!(?mode, "spitfire.layout.set");
                self.workspaces.active_mut().tiling.params.set_mode(mode);
                self.arrange_tiling();
            }

            ConfigCommand::LayoutCycle => {
                self.workspaces.active_mut().tiling.params.cycle_mode();
                info!(mode = ?self.workspaces.active().tiling.params.mode, "spitfire.layout.cycle");
                self.arrange_tiling();
            }

            ConfigCommand::MfactInc(delta) => {
                self.workspaces.active_mut().tiling.params.inc_mfact(delta);
                info!(mfact = self.workspaces.active().tiling.params.mfact, "spitfire.mfact.inc");
                self.arrange_tiling();
            }

            ConfigCommand::NmasterInc(delta) => {
                self.workspaces.active_mut().tiling.params.inc_nmaster(delta);
                info!(nmaster = self.workspaces.active().tiling.params.nmaster, "spitfire.nmaster.inc");
                self.arrange_tiling();
            }

            ConfigCommand::WorkspaceFocus(n) => {
    ...
    /// Runs every `spitfire.autostart { ... }` entry once, right after the
    ...
    /// `spitfire.reload()` / (Phase 3) `spitfirectl reload`: drops the
    /// current Lua state and re-runs the config file from scratch. Falls
    /// back to keeping the old config untouched if the new one fails to
    /// load, so a typo never leaves the compositor without any bindings.
    pub fn reload_config(&mut self) {
        ...
        match spitfire_config::Config::load(&path) {
            ...
            Err(err) => {
                error!(path = %path.display(), %err, "Failed to reload config, keeping the previous one");
            }
        }
    }
    ...
    fn keyboard_key_to_action<B: InputBackend>(&mut self, evt: B::KeyboardKeyEvent) -> KeyAction {
        ...
        let action = keyboard
            .input(self, keycode, state, serial, time, |cstate, modifiers, handle| {
                ...
                if let KeyState::Pressed = state {
                    if !inhibited {
                        ...
                        // user config can freely redefine any of them.
                        let config_mods = spitfire_config::Modifiers {
                            logo: modifiers.logo,
                            ctrl: modifiers.ctrl,
                            alt: modifiers.alt,
                        ...
                        let action = cstate
                            .config
                            .find_bind(config_mods, keysym)
                            .map(KeyAction::LuaBind)
    ...
    fn update_keyboard_focus(&mut self, location: Point<f64, Logical>, serial: Serial) {
        // While the session is locked, keyboard focus is pinned to the lock
        // surface (set once in the SessionLockHandler::new_surface — see
        // shell/mod.rs) and must never be stolen back by a click on
        // whatever's still mapped underneath it.
        if self.locked {
            return;
        }
...
impl<BackendData: Backend> SpitfireState<BackendData> {
    pub fn process_input_event_windowed<B: InputBackend>(&mut self, event: InputEvent<B>, output_name: &str) {
        match event {
            ...
            _ => (), // other events are not handled in spitfire (yet)
        }
    }
...
impl SpitfireState<UdevData> {
    pub fn process_input_event<B: InputBackend>(&mut self, dh: &DisplayHandle, event: InputEvent<B>) {
        match event {
            ...
            _ => {
                // other events are not handled in spitfire (yet)
            }
        }
    }
...
enum KeyAction {
    ...
    ToggleTint,
    ToggleDecorations,
    /// A `spitfire.bind(...)` from the Lua config matched — `usize` is its
    /// index in `Config`'s bind list (see `Config::find_bind`).
    LuaBind(usize),
    /// Do nothing more
    None,
}
