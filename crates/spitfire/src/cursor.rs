//! Loads the XCursor theme so `X11Wm::set_cursor` has a default pointer
//! image to hand XWayland clients that don't set their own — Wayland
//! clients get their cursor image from `wl_pointer.set_cursor` instead and
//! never touch this. Adapted from anvil's own `cursor.rs` — see
//! `NOTICE.md`. Only used when the `xwayland` feature is on.
