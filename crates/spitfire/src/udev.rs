...
use crate::{
    ...
    render::*,
    shell::WindowElement,
    state::{take_presentation_feedback, update_primary_scanout_output, Backend, SpitfireState},
};
use crate::{
...
impl DmabufHandler for SpitfireState<UdevData> {
    ...
    fn dmabuf_imported(
            ...
            .is_ok()
        {
            ...
            let _ = notifier.successful::<SpitfireState<UdevData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(SpitfireState<UdevData>);
...
pub fn run_udev() {
     ...
     */
    let primary_gpu = if let Ok(var) = std::env::var("SPITFIRE_DRM_DEVICE") {
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<SpitfireState<UdevData>>();
                    }
                    data.handle
    ...
    let global = dmabuf_state.create_global_with_default_feedback::<SpitfireState<UdevData>>(
        &display_handle,
        &default_feedback,
    );
        ...
        .and_then(|x| x.ok())
    {
        if let Some(backend) = state.backend_data.backends.get(&primary_node) {
            ...
            if supports_syncobj_eventfd(&import_device) {
                let syncobj_state =
                    DrmSyncobjState::new::<SpitfireState<UdevData>>(&display_handle, import_device);
...
impl DrmLeaseHandler for SpitfireState<UdevData> {
...
delegate_drm_lease!(SpitfireState<UdevData>);

impl DrmSyncobjHandler for SpitfireState<UdevData> {
    fn drm_syncobj_state(&mut self) -> Option<&mut DrmSyncobjState> {
        self.backend_data.syncobj_state.as_mut()
    }
}
smithay::delegate_drm_syncobj!(SpitfireState<UdevData>);
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<SpitfireState<UdevData>>(global);
        }
    }
}
...
impl SpitfireState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let registration_token = self
            ...
            .insert_source(
                notifier,
                move |event, metadata, data: &mut SpitfireState<_>| match event {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<SpitfireState<UdevData>>(
                    &self.display_handle,
                    &node,
                )
                .inspect_err(|err| {
    ...
    fn connector_connected(
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<SpitfireState<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<SpitfireState<UdevData>>();
            }
    ...
    fn render_surface(&mut self, node: DrmNode, crtc: crtc::Handle, frame_target: Time<Monotonic>) {
        ...
        // No busy render loop to piggyback on here the way winit.rs does
        // (rendering is fully event/timer-driven under DRM/KMS) — so this
        // is re-tiling's natural home instead: right before every frame
        // actually gets composited. Also prunes dead windows from the
        // tiling order, same as it does under winit.
        self.arrange_tiling();
...
fn render_surface<'a>(
    ...
    // The optional built-in bar (Phase 8) — same as winit.rs, pushed into
    // the same element list as the cursor/dnd icon above rather than
    // threaded through output_elements like spitfire.border, since it
    // always sits on top with no occlusion-avoidance concerns. Hidden
    // while locked: it must never show live clock/workspace info over the
    // lock screen.
